...
function init() {
    ...
    document
        ...
        .addEventListener("change", (ev) => loadProgressFromFile(ev));
            ...
            if (optTaken[index]) {
                ...
                characterDataToSort = characterDataToSort.filter((char) => {
                    if (!(opt.key in char.opts))
                        console.warn(
                            `Warning: ${opt.key} not set for ${char.name}.`
                        );
                    return (
...
function pick(sortType) {
    ...
    if (leftInnerIndex < leftListLen && rightInnerIndex === rightListLen) {
        ...
        leftInnerIndex === leftListLen &&
        rightInnerIndex < rightListLen
    ) {
    ...
    // Revokes file if it already exists
    if (textFile !== null) {
        window.URL.revokeObjectURL(textFile);
    }

    textFile = window.URL.createObjectURL(data);
    const saveBtn = document.querySelector(".sorting.save.button");
    saveBtn.setAttribute("href", textFile);
    saveBtn.setAttribute("download", `wfsSaveFile${battleNo}`);
...
/**
 * Loads progress from save file.
 * @param {*} event - File chosen by user to load
 */
...
function decodeQuery(queryString = window.location.search.slice(1)) {
    ...
    try {
        ...
        const beforeDateIndex = dateMap.reduce(
            (prevIndex, currDate, currIndex) => {
                return currDate.val < seedDate.val ? currIndex : prevIndex;
            },
            -1
        );
        const afterDateIndex = dateMap.findIndex(
            (date) => date.val > seedDate.val
        );
        ...
        options.forEach((opt, index) => {
            if ("sub" in opt) {
                ...
                document.getElementById(`cbgroup-${opt.key}`).checked =
                    optIsTrue;
                opt.sub.forEach((subopt, subindex) => {
...
function preloadImages() {
    ...
    return Promise.all(
        characterDataToSort.map(async (char, idx) => {
            characterDataToSort[idx].img = await loadImage(
                imageRoot + char.img
            );
        })
    );
